        ...
        private async Task SendNotificationsForMembers(List<MemberWithProjecsNotifications> membersWithProjecsNotifications, string subjectName, string baseUrl)
        {
            if (membersWithProjecsNotifications.Any())
            {
                foreach (var memberWithProjecsNotifications in membersWithProjecsNotifications)
                {
                    ...
                    sb.Append($@"<p>Hello, {memberWithProjecsNotifications.MemberFullName}!<br>");

                    var dayOrDays = string.Empty;
                    ...
                    {
                        var editionDaysFormat = "<b>" + string.Join(", </b><b>", project.EditionDays.EditionDays.Select(x => x.ToString(dateFormatShort, CultureInfo.InvariantCulture))) + "</b>";

                        if (memberWithProjecsNotifications.ProjectsWithDatesEditing.Count == 1)
                        {
                            dayOrDays = "day";
                            sb.Append($"<p>This is a friendly reminder, that you haven’t entered your Time Entries on <b>{project.ProjectWithDatesEditing.Name}</b> project for <b>{project.EditionDays.EditionDays.Length}</b> work{dayOrDays}: {editionDaysFormat}.<br>");
                        }
                        else
                        {
                            dayOrDays = "days";
                            sb.Append("<p>This is a friendly reminder, that you haven’t entered your Time Entries on on the following projects:");
                            sb.Append($"{++indexCurrentProject}. <b>{project.ProjectWithDatesEditing.Name}</b> project for <b>{project.EditionDays.EditionDays.Length}</b> work{dayOrDays}: {editionDaysFormat}.<br>");
                        }
                    }

                    sb.Append($"<p><a href=\"{baseUrl}/calendar/\">Would you like to enter your time now?</a><br>");
